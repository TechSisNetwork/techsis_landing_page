    ...
    <Container>
      ...
      <div className="mt-10 grid gap-5 md:grid-cols-3">
        {stories.map((story, i) => <article key={story.name} className={`rounded-xl border border-gray-100 p-7 shadow-sm ${i === 1 ? "bg-[#151515] text-white" : "bg-white"}`}>
          ...
          <p className="min-h-28 text-base leading-6 opacity-80">“{story.quote}”</p>
          <div className="mt-6 flex items-center gap-3"><Image src={portraits} alt="" width={42} height={42} className="h-10 w-10 rounded-full object-cover object-left"/><div><strong className="block text-base">{story.name}</strong><small className="text-sm text-primary">{story.role}</small></div></div>
        </article>)}
      </div>
    </Container>
  </section>
...
export function CTA({ title = "Ready to be part of the movement?" }: { title?: string }) {
  return <div className="mx-auto my-16 max-w-3xl rounded-2xl bg-[#fff7f9] p-10 text-center">
    ...
    <p className="mt-3 text-base text-gray-500">Join women building careers, community, and impact in tech across Africa and beyond.</p>
