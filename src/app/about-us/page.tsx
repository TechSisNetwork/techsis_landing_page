...
export default function AboutPage() {
  return <main>
    <section className="about-hero">
      <Container className="grid items-center gap-12 lg:grid-cols-[1.08fr_.92fr]">
        <div>
          ...
          <h1 className="mt-3 max-w-[650px] text-[42px] font-bold leading-[1.02] tracking-[-.04em] text-[#181818] md:text-[58px]">Building a Culture<br/>Where Women in<br/>Tech Grow Together</h1>
          <p className="mt-7 max-w-[570px] text-xl leading-[1.25] text-[#999]">Techsis Network is building a community-driven ecosystem where women can learn, collaborate, gain experience, and confidently grow their careers in technology.</p>
          ...
          <div className="mt-10 flex gap-12">{[["100+","Women Trained"],["50+","Internship Placements"],["83%","Career Transition Rate"]].map(x=><div key={x[1]}><strong className="text-2xl">{x[0]}</strong><small className="block text-[10px] text-gray-500">{x[1]}</small></div>)}</div>
        </div>
        <div className="relative mx-auto"><div className="absolute -left-10 top-14 h-36 w-36 rounded-full bg-[#f0fbfb]"/><Image src={hero} alt="Women collaborating in technology" className="relative w-[460px] rounded-[24px]" priority/><div className="absolute -bottom-5 -left-8 rounded-lg bg-white px-4 py-3 text-[10px] shadow-xl"><b>5+ Partner Orgs</b><br/><span className="text-gray-400">NGOs, Tech & Startups</span></div><div className="absolute -right-8 top-5 rounded-lg bg-white px-4 py-3 text-[10px] shadow-xl">Cohort 3 — Open<br/><b className="text-primary">Applications Live →</b></div></div>
      </Container>
    </section>

    <section className="bg-[#fff6f8] py-20 md:py-28"><Container className="grid items-center gap-14 md:grid-cols-[320px_1fr]"><Image src={story} alt="Obelema Fubara, founder" className="mx-auto w-[300px]"/><div><p className="eyebrow normal-case tracking-normal">Our Story</p><h2 className="mt-3 text-4xl font-bold leading-[1.05]">Born from a Gap That Couldn&apos;t<br className="hidden lg:block"/> Be Ignored</h2><div className="mt-6 max-w-[780px] space-y-5 text-[15px] leading-[1.25] text-[#888]"><p>Techsis Network was founded after witnessing firsthand how talented African women were being locked out of the tech industry, not because of lack of skill, but because of lack of access and experience. No internships. No mentors. No networks. No pathways.</p><p>Our founder, having navigated those same barriers, built Techsis to be the bridge she never had. A place where women don’t just learn, they practice, get placed, get paid, and get ahead.</p><p>Today, Techsis Network is a growing ecosystem of women in product management, software development, data, design, and business analysis, all building real careers with real support.</p></div></div></Container></section>
    ...
    <section className="bg-[#fff6f8] py-24"><Container><div className="section-heading"><span className="normal-case tracking-normal">The People Behind the Mission</span><h2>Our Leadership Team</h2></div><div className="mx-auto mt-12 grid max-w-4xl gap-6 md:grid-cols-3">{team.map(t=><article className="overflow-hidden rounded-xl bg-white shadow-sm" key={t[1]}><Image src={`/${t[0]}`} alt={t[1]} width={285} height={200} className="h-[210px] w-full object-cover"/><div className="p-4"><h3 className="font-bold">{t[1]}</h3><p className="text-xs font-semibold text-primary">{t[2]}</p><p className="mt-3 min-h-16 text-[10px] leading-[1.3] text-gray-500">{t[3]}</p><span className="mt-3 block text-[10px] text-teal-700">▣ LinkedIn</span></div></article>)}</div></Container></section>
